...
class ExportPost extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        $template = '"{{title}}","{{faq_answer}}","{{identifier}}","{{tags}}","{{status}}"' .
            ',"{{show_on_main}}","{{faq_order}}","{{t_title}}","{{t_identifier}}","{{t_status}}","{{t_show_on_main}}","{{t_topic_order}}","{{store_id}}"';
        $content = $headers->toString($template);
        $content .= "\n";

        $collection = $this->_objectManager->create(
            'FME\Faqs\Model\ResourceModel\Faqs\Collection'
        )->joinTopicTable()->joinStoreTable();
        ...
        return $this->fileFactory->create('faqs.csv', $content, DirectoryList::VAR_DIR);
    }
    ...
    {
        return $this->_authorization->isAllowed('FME_Faqs::importexport');
    }
